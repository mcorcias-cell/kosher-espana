...
const S = {
  ...
  sidebarTitulo: { fontWeight: 700, color: '#2d3748', fontSize: '0.9rem', marginBottom: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em' },
  catBtn: (activa) => ({ display: 'flex', alignItems: 'center', gap: '8px', width: '100%', textAlign: 'left', padding: '8px 10px', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '0.875rem', fontWeight: activa ? 700 : 400, background: activa ? '#ebf8ff' : 'transparent', color: activa ? '#2b6cb0' : '#4a5568', marginBottom: '2px' }),
  catCount: { marginLeft: 'auto', fontSize: '0.75rem', color: '#a0aec0', background: '#f7fafc', padding: '1px 6px', borderRadius: '10px' },
  ...
  categoriaActiva: { display: 'inline-flex', alignItems: 'center', gap: '6px', padding: '4px 12px', background: '#ebf8ff', color: '#2b6cb0', borderRadius: '20px', fontSize: '0.85rem', fontWeight: 600, marginBottom: '1rem' },
};
...
const BusquedaPage = () => {
  ...
  const buscar = useCallback(async (pag = 1, catId = categoriaActiva) => {
    setCargando(true);
    try {
  ...
  }, [filtros, categoriaActiva]);

  const handleCategoriaClick = (cat) => {
    if (!cat || categoriaActiva === cat.id) {
      setCategoriaActiva(null);
      setCategoriaActivaNombre('');
      buscar(1, null);
    } else {
      setCategoriaActiva(cat.id);
      setCategoriaActivaNombre(`${cat.icono} ${cat.nombre}`);
      buscar(1, cat.id);
    }
  };
        ...
        <div style={S.sidebar}>
          ...
          {categorias.map(cat => (
            <button key={cat.id} style={S.catBtn(categoriaActiva === cat.id)} onClick={() => handleCategoriaClick(cat)}>
              <span>{cat.icono}</span>
              <span style={{flex:1, fontSize:'0.82rem'}}>{cat.nombre}</span>
